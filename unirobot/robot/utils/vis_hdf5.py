...
class DataVisualizer:
    ...
    def _validate_data(self):
        """验证数据形状一致性"""
        assert (
            self.data["top_img"].shape[0]
            == self.data["hand_img"].shape[0]
            == self.data["action"].shape[0]
        ), "帧数不匹配"
    ...
    def _update_plots(self, frame_idx: int):
        ...
        for i in range(ACTION_DIM):
            ...
            self.lines[i].set_data(
                range(frame_idx + 1), action_data[: frame_idx + 1, i]
            )
    ...
    def visualize(self):
        ...
        while True:
            ...
            if top_img.shape[0] < max_height:
                padding = np.zeros(
                    (max_height - top_img.shape[0], top_img.shape[1], 3), dtype=np.uint8
                )
                top_img = np.vstack([top_img, padding])
            if hand_img.shape[0] < max_height:
                padding = np.zeros(
                    (max_height - hand_img.shape[0], hand_img.shape[1], 3),
                    dtype=np.uint8,
                )
            ...
            if info_panel.shape[1] != top_row.shape[1]:
                info_panel = cv2.resize(
                    info_panel, (top_row.shape[1], info_panel.shape[0])
                )
...
def create_sample_data(num_frames=100):
    ...
    with h5py.File("./episode_0001.hdf5", "r") as fin:
        ...
        print(action.shape)
    return {"top_img": top_img[()], "hand_img": hand_img[()], "action": action[()]}
