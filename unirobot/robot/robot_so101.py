...
def enter_pressed(key):
    ...
    return select.select([sys.stdin], [], [], 0)[0] and sys.stdin.readline().strip()==key
...
class So101(BaseRobot):
    ...
    def __init__(
        ...
        teleoperator_cfg: Optional[Union[Dict[str,Dict[str, Any]],Dict[str, Any]]] = None,
        ...
        try:
            ...
            while True:
                ...
                if self._start_colloct:
                    ...
                    self._colloct_data['action'].append(np.array(list(actual_action.values())))
                    self._frame_count += 1
                if enter_pressed('s'):
    ...
    def _save_data(self,):
        ...
        episode_file = os.path.join(task_data_dir,f"episode_{self._count_episode:04d}.hdf5")
        ...
        with h5py.File(episode_file, 'w') as f:
            # 创建数据集
            f.create_dataset('top_imgs', data=top_imgs, compression='gzip', compression_opts=4)
            f.create_dataset('hand_imgs', data=hand_imgs, compression='gzip', compression_opts=4)
            f.create_dataset('action', data=action, compression='gzip', compression_opts=4)
            # 添加元数据
