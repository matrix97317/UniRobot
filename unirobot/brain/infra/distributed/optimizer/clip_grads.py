...
# pylint: skip-file
# flake8: noqa
import amp_C  # noqa: I900
import torch
from apex.multi_tensor_apply import multi_tensor_applier  # noqa: I900
from distutils.version import LooseVersion

if LooseVersion(torch.__version__) >= LooseVersion("2.0.0"):
    from torch import inf
else:
    from torch._six import inf  # type: ignore[no-redef]


def clip_grad_norm_fp32(
    parameters, grads_for_norm, max_norm, norm_type=2, model_parallel_group=None
):
    """Clips gradient norm of parameters whose gradients are in fp32.
    ...
    for param in parameters:
        if param.grad is not None:
            if param.grad.type() != "torch.cuda.FloatTensor":
                raise ValueError("param.grad.type() != `torch.cuda.FloatTensor`")
