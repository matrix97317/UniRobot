...
def custom_backward(output, grad_output, is_group_grad=False, use_cpu_offload=False):
    ...
    if not is_group_grad:
        ...
        if not isinstance(output[0], torch.Tensor):
            raise ValueError(f"output == ' {type(output).__name__} '.")
        if not isinstance(grad_output[0], (torch.Tensor, type(None))):
            raise ValueError(f"grad_output == ' {type(grad_output).__name__}'.")
            ...
            with torch.autograd.graph.save_on_cpu(pin_memory=True):
                if amp_scaler is not None:
                    with torch.cuda.amp.autocast(enabled=True):
                        ...
                        amp_scaler.scale(torch.ones((1)).cuda())
                else:
                    # with torch.autograd.graph.save_on_cpu(pin_memory=True):
                    output_tensor = model(input_tensor)  # model recive tensor list
        else:
            if amp_scaler is not None:
                with torch.cuda.amp.autocast(enabled=True):
                    output_tensor = model(input_tensor)
...
def backward_step(
    optimizer, input_tensor, output_tensor, output_tensor_grad, use_cpu_offload
):
    ...
    # Backward pass.
    if len(output_tensor_grad) == 1 and len(output_tensor) == 1:
        if output_tensor_grad[0] is None:
            output_tensor = [optimizer.scale_loss(output_tensor[0])]
        custom_backward(
            output_tensor[0], output_tensor_grad[0], use_cpu_offload=use_cpu_offload
        )
    else:
        if len(output_tensor) == len(output_tensor_grad):
            if len(output_tensor) > 1:
                ...
                for out_tensor, out_tensor_grad in zip(
                    output_tensor, output_tensor_grad
                ):
                    custom_backward(
                        out_tensor, out_tensor_grad, use_cpu_offload=use_cpu_offload
                    )
        else:
            raise ValueError(
                f"output tensor length {len(output_tensor)} "
                f"!= output tensor grad length {len(output_tensor_grad)}"
            )
...
def forward_backward_no_pipelining(
    ...
    with context_handler():
        for i in range(num_microbatches - 1):
            ...
            if profiler is not None:
                profiler.record_time_cost("forward_step")
            if profiler is not None:
                profiler.tic()
            if not forward_only:
                backward_step(
            ...
            if profiler is not None:
                profiler.record_time_cost("backward_step")
...
def get_out_tensor_shapes(rank, model_tensor_shape_list):
    ...
    if rank < 0 or rank == get_pipeline_model_parallel_world_size():
        ...
        if isinstance(model_tensor_shape_list[rank], tuple):
            tensor_shapes.append(model_tensor_shape_list[rank])
        elif isinstance(model_tensor_shape_list[rank], list) and isinstance(
            model_tensor_shape_list[rank][0], tuple
        ):
            tensor_shapes = model_tensor_shape_list[rank]
        else:
            raise ValueError(
                f"rank {rank}, out tensor shapes not be supported. "
                f"{model_tensor_shape_list[rank]}"
            )
    return tensor_shapes
...
def forward_backward_pipelining_with_1F1B(
    ...
    for i in range(num_warmup_microbatches):
        ...
        if profiler is not None:
            profiler.record_time_cost("recv_forward")
        if profiler is not None:
            profiler.tic()
        output_tensor = forward_step(
            data_loader,
        ...
        if profiler is not None:
            profiler.record_time_cost("send_forward")
    ...
    for i in range(num_microbatches_remaining):
        ...
        if forward_only:
            ...
            if profiler is not None:
                profiler.record_time_cost("send_forward_recv_backward")
            ...
            if last_iteration:
                ...
                if profiler is not None:
                    profiler.record_time_cost("send_backward")
                ...
                if profiler is not None:
                    profiler.record_time_cost("send_backward_recv_forward")
    ...
    if not forward_only:
        for i in range(num_warmup_microbatches):
            ...
            if profiler is not None:
                profiler.record_time_cost("recv_backward")
            if profiler is not None:
                profiler.tic()
            input_tensor_grad = backward_step(
                optimizer,
