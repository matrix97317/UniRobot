# -*- coding: utf-8 -*-
"""UniRobot Megatron-DataParallel. DataParallel policy."""
# pylint: skip-file
# flake8: noqa
import logging
import math
...
class MemoryBuffer:
    ...
    def get(self, shape, start_index):
        ...
        if end_index > self.numel:
            logger.error("requested tensor is out of the buffer range.")
        buffer_tensor = self.data[start_index:end_index]
        buffer_tensor = buffer_tensor.view(shape)
        return buffer_tensor


class DistributedDataParallelBase(torch.nn.Module, ABC):
...
class DistributedDataParallel(DistributedDataParallelBase):
    ...
    def __init__(
        ...
        if self.accumulate_allreduce_grads_in_fp32:
            if not self.use_contiguous_buffers:
                logger.error("you muse set use_contiguous_buffers=True")
    ...
    def zero_grad_buffer(self):
        ...
        if self._grad_buffers is None:
            logger.error("buffers are not initialized.")
    ...
    def allreduce_gradients(self):
        ...
        if self._grad_buffers is not None:
            for _, buffer_ in self._grad_buffers.items():
                buffer_.data /= get_data_parallel_world_size()
                torch.distributed.all_reduce(
                    buffer_.data, group=get_data_parallel_group()
                )
        else:
            ...
            for tp in buckets:
                ...
                coalesced /= get_data_parallel_world_size()
                torch.distributed.all_reduce(coalesced, group=get_data_parallel_group())
